...
- (IBAction)save:(id)sender {
  GCDataPoint * dp = [GCDataPoint newDataPointInDefaultContext];
  
  dp.date = [NSDate date];

  dp.dip = [NSNumber numberWithFloat:self.dip];
  dp.dipDir = [NSNumber numberWithFloat:self.dipDir];
  dp.latitude = [NSNumber numberWithFloat:latitude];
  dp.longitude = [NSNumber numberWithFloat:longitude];
  
  NSLog(@"saved %@",[dp encodedString]);
}
...
{  
  ...
  dip = atan2(downV.z,dipV.length())*180./M_PI+90.;
  dipLabel.text = [NSString stringWithFormat:@"%.1f",dip];
  ...
  dipDir = dipDirection*(180/M_PI);
  dipDirLabel.text = [NSString stringWithFormat:@"%.1f",dipDir];
